...
<head>
    ...
    <title><?= esc($pageTitle ?? 'Admin Audit Trail') ?> | MeralQ</title>
...
<body>
    <?php
        ...
        $baseUrl = site_url('dashboard/admin/audit-trail');
    ?>
    ...
    <main class="container py-5">
        <section class="hero-card p-4 p-lg-5 mb-4">
            <span class="pill">Admin Audit Trail</span>
            <h1 class="display-6 fw-bold mt-3">Browse audit activity without stretching the main dashboard.</h1>
            <p class="mb-0 text-white-50">This page keeps the full account and sign-in history on its own screen, with paging so the layout stays stable as the log grows.</p>
        </section>
        ...
        <section class="card panel-card p-4">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
                ...
                <a class="btn btn-outline-dark" href="<?= site_url('dashboard/admin') ?>">Back to Dashboard</a>
            </div>
            ...
            <div class="table-responsive">
                <table class="table align-middle mb-0">
                    ...
                    <tbody>
                        <?php if ($logs === []): ?>
                            <tr>
                                <td colspan="4" class="empty-state">No audit entries available.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($logs as $entry): ?>
                                <?php $displayName = ! empty($entry['fullname']) ? sprintf('%s (%s)', $entry['fullname'], $entry['username'] ?? 'n/a') : 'Unknown user'; ?>
                                <tr>
                                    <td>
                                        <div class="fw-semibold"><?= esc($displayName) ?></div>
                                        <div class="text-secondary small text-uppercase"><?= esc($entry['role'] ?? 'unknown') ?></div>
                                    </td>
                                    <td><span class="badge text-bg-light border"><?= esc($entry['action']) ?></span></td>
            ...
            <?php if ($pageCount > 1): ?>
                <nav class="mt-4" aria-label="Admin audit trail pages">
